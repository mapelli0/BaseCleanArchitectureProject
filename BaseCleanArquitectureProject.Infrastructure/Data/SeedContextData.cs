...
namespace BaseCleanArquitectureProject.Infrastructure.Data {
	...
	public static class SeedContextData {
		public static async Task SeedData (this BaseCleanArquitectureProjectDbContext dbContext, CancellationToken cancellationToken) {
			await SeedRolesAsync(dbContext, cancellationToken);
			await SeedAdministratorUser(dbContext, cancellationToken);
			await dbContext.SaveChangesAsync(cancellationToken);
		}

		private static async Task SeedAdministratorUser (BaseCleanArquitectureProjectDbContext dbContext, CancellationToken cancellationToken) {
			var userStore = new UserStore<ApplicationUser, ApplicationRole, BaseCleanArquitectureProjectDbContext, Guid>(dbContext);
		...
		private static async Task SeedRolesAsync (BaseCleanArquitectureProjectDbContext dbContext, CancellationToken cancellationToken) {
			var roleS = new RoleStore<ApplicationRole, BaseCleanArquitectureProjectDbContext, Guid>(dbContext);
