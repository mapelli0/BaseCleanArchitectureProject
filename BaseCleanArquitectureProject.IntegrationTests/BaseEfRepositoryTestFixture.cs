...
using Autofac;
using AutoMapper;
using BaseCleanArquitectureProject.Infrastructure;
...
namespace BaseCleanArquitectureProject.IntegrationTests {

	public class BaseEfRepositoryTestFixture {
		protected BaseCleanArquitectureProjectDbContext _dbContext;
		protected Mock<IMediator> _dispatcherMock;
		protected IMediator _dispatcher;

		protected static DbContextOptions<BaseCleanArquitectureProjectDbContext> CreateNewContextOptions() {
			...
			var dbBuilder = new DbContextOptionsBuilder<BaseCleanArquitectureProjectDbContext>();
			dbBuilder.UseInMemoryDatabase("BaseCleanArquitectureProject").UseInternalServiceProvider(serviceDBProvider);
		...
		protected async Task<TRepository> GetRepositoryWithMocks<TRepository, TEntity>() {
			...
			_dbContext = new BaseCleanArquitectureProjectDbContext(options, _dispatcherMock.Object);
		...
		protected async Task<TRepository> GetRepository<TRepository, TEntity>() {
			...
			_dbContext = new BaseCleanArquitectureProjectDbContext(options, _dispatcher);
